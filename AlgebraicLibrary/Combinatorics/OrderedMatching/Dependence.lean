...
theorem list_perm_prod_apply_eq_of_forall_mem_eq {α : Type*}
    (l : List (Equiv.Perm α)) (a : α) (h : ∀ σ ∈ l, σ a = a) :
    l.prod a = a := by
  induction l with
  | nil => simp
  | cons σ l ih =>
...
/-- An edge swap fixes every point outside that edge. -/
theorem orderedMatching_edgeSwap_apply_of_ne {α : Type*} [DecidableEq α]
    ...
    M.edgeSwap r a = a := by
  simpa [OrderedMatching.edgeSwap] using
    (Equiv.swap_apply_of_ne_of_ne hl hr)

/-- Every matching-cube element fixes an unmatched point. -/
theorem orderedMatching_tau_apply_of_unmatched {α : Type*} [DecidableEq α]
    (M : OrderedMatching α) (x : M.EdgeCube) {a : α}
...
theorem orderedMatching_edgePerm_apply_left_of_ne {α : Type*} [DecidableEq α]
    (M : OrderedMatching α) (x : M.EdgeCube)
  ...
  · have hdisj := M.edges_disjoint (r := s) (s := r) hsr
    ...
    simp [OrderedMatching.edgePerm, OrderedMatching.edgeSwap, hx,
...
theorem orderedMatching_edgePerm_preserves_edge {α : Type*} [DecidableEq α]
    (M : OrderedMatching α) (x : M.EdgeCube)
    (s r : Fin M.edgeCount) {a : α} (ha : M.InEdge r a) :
...
theorem orderedMatching_edgePerm_inv {α : Type*} [DecidableEq α]
  ...
  by_cases hx : x r
  · simp [OrderedMatching.edgePerm, OrderedMatching.edgeSwap, hx, Equiv.swap_inv]
  · simp [OrderedMatching.edgePerm, hx]

theorem orderedMatching_edgePerm_apply_eq_of_same_bit_on_edge
    {α : Type*} [DecidableEq α] (M : OrderedMatching α)
    {x y : M.EdgeCube} {r s : Fin M.edgeCount} {a : α}
...
theorem orderedMatching_list_prod_apply_eq_of_same_bit_on_edge
    {α : Type*} [DecidableEq α] (M : OrderedMatching α)
    (l : List (Fin M.edgeCount)) {x y : M.EdgeCube}
    {r : Fin M.edgeCount} {a : α} (hxy : x r = y r) (ha : M.InEdge r a) :
...
/-- On an endpoint of edge `r`, `tau x` depends only on the `r`th bit. -/
theorem orderedMatching_tau_apply_eq_of_same_bit_on_edge
    {α : Type*} [DecidableEq α] (M : OrderedMatching α)
    {x y : M.EdgeCube} {r : Fin M.edgeCount} {a : α}
    (hxy : x r = y r) (ha : M.InEdge r a) :
...
/-- The inverse of `tau x` is the reversed product of its edge factors. -/
theorem orderedMatching_tau_symm_eq_reverse_prod {α : Type*} [DecidableEq α]
  ...
  calc
    ...
    _ = ((List.ofFn fun r : Fin M.edgeCount => M.edgePerm x r).prod)⁻¹ := by rfl
    _ =
        (List.map (fun σ : Equiv.Perm α => σ⁻¹)
    ...
    _ = ((List.finRange M.edgeCount).reverse.map fun r => M.edgePerm x r).prod := by
      rw [List.ofFn_eq_map, ← List.map_reverse]

theorem orderedMatching_tau_symm_apply_eq_of_same_bit_on_edge
    {α : Type*} [DecidableEq α] (M : OrderedMatching α)
    {x y : M.EdgeCube} {r : Fin M.edgeCount} {a : α}
    (hxy : x r = y r) (ha : M.InEdge r a) :
...
/-- The inverse matching-cube element also fixes every unmatched point. -/
