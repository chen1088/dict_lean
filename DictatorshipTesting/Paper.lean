/-
Paper-level import index.

`PaperAux` checks helper-only modules, while the imports below expose the
paper-numbered statements.  Numbered files should stay as the stable place a
reader looks for the corresponding theorem, even when the proof is delegated to
an auxiliary input file.
-/
