...
theorem S05_matchingHighIdempotent_idempotent
  ...
  rw [← (S05_Int_PMConvolution M (S05_matchingHighIdempotent M F)).2]
  change
  ...
  have hproj :
    ...
    calc
      matchingLocalProjection M (S05_matchingLowIdempotent M F) =
          ...
          exact (S05_Int_PMConvolution M (S05_matchingLowIdempotent M F)).1
      _ = S05_matchingLowIdempotent M F := by
          exact S05_matchingLowIdempotent_idempotent M F
  change
    matchingHighConvolution M (S05_matchingLowIdempotent M F) =
      fun _ => 0
  rw [← (S05_Int_PMConvolution M (S05_matchingLowIdempotent M F)).2]
  funext π
  rw [hproj]
  simp
...
theorem S05_matchingLow_add_matchingHigh
  ...
  have hlow := (S05_Int_PMConvolution M F).1
  have hhigh := (S05_Int_PMConvolution M F).2
  funext π
