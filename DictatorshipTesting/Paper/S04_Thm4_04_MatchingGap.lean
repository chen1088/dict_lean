...
-/
/-!
# Theorem 4.4: Matching-cube spectral gap

This is `thm:matching-gap` from `dictatorship_testing_stoc27_shortened.tex`.
-/
...
/-- Theorem 4.4, `thm:matching-gap`: matching-cube spectral gap.  The active
...
theorem S04_Thm4_04_MatchingGap
    (n : ℕ) (hn : 4 ≤ n)
    (F : Perm (Fin n) → ℝ) :
      ...
      exact
        spectralGapFromBlockModel_withDim
          ...
          (fun lam hrow hstandard =>
            S05_Lem5_19_tableau_even_certificate m hm lam hrow hstandard)
    constructor
    · have hscale :
    ...
    have hgap16 : MatchingSpectralGapConstant (2 * m + 1) (1 / 6 : ℝ) := by
      exact
        spectralGapFromBlockModel_withDim
          ...
          (fun lam hrow hstandard =>
            S05_Lem5_20_tableau_odd_certificate m hm lam hrow hstandard)
    constructor
    · exact hgap16 F
    · intro heven
