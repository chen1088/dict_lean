...
import DictatorshipTesting.Paper.S05_Lem5_28_EvenSpectralBridge
import DictatorshipTesting.Paper.S05_Lem5_29_OddSpectralBridge
...
theorem S04_Thm4_08_MatchingGap
    (n : ℕ) (hn : 4 ≤ n)
    (F : Perm (Fin n) → ℝ) :
  ...
  · rcases (even_iff_exists_two_mul.mp heven) with ⟨m, rfl⟩
    ...
    have hgap15 : MatchingSpectralGapConstant (2 * m) (1 / 5 : ℝ) := by
      exact S05_Lem5_28_tableauDim_evenSpectralGapFromCertificates
        m hm (spectralBlockModelInputWithDim_even_from_section5 m hm)
    constructor
    · have hscale :
  ...
  · have hnotEven : ¬ Even n := Nat.not_even_iff_odd.mpr hodd
    ...
    have hgap16 : MatchingSpectralGapConstant (2 * m + 1) (1 / 6 : ℝ) := by
      exact S05_Lem5_29_tableauDim_oddSpectralGapFromCertificates
        m hm (spectralBlockModelInputWithDim_odd_from_section5 m hm)
    constructor
    · exact hgap16 F
    · intro heven
