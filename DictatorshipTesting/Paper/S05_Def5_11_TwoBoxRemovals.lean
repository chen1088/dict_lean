...
- `DictatorshipTesting`
- `DictatorshipTesting.Paper.S05_Def5_12_SignedTwoBoxRemovals`
-/


/-!
Paper statement: Definition 5.11 (`def:two-box-removals`)
...
/-- Definition 5.9 row fact: a horizontal two-box removal has total row
difference `2`. -/
theorem S05_Def5_11_sum_row_diff_of_horizontalTwoBoxRemoval
...
/-- Definition 5.9 row fact: a vertical two-box removal has total row
difference `2`. -/
theorem S05_Def5_11_sum_row_diff_of_verticalTwoBoxRemoval
    {n k : Nat} {lam : YoungDiagram n} {mu : YoungDiagram k}
    (h : S05_IsVerticalTwoBoxRemoval lam mu) :
    (Finset.range n).sum (fun i => youngRow lam i - youngRow mu i) = 2 := by
  exact sum_row_diff_of_verticalTwoStripChild h

/-- Definition 5.9 row fact: horizontal two-box children are rowwise bounded by
their parent. -/
theorem S05_Def5_11_row_le_parent_of_horizontalTwoBoxRemoval
    {n k : Nat} {lam : YoungDiagram n} {mu : YoungDiagram k}
    (h : S05_IsHorizontalTwoBoxRemoval lam mu) (i : Nat) :
    youngRow mu i <= youngRow lam i :=
  row_le_parent_of_horizontalTwoStripChild h i

/-- Definition 5.9 row fact: vertical two-box children are rowwise bounded by
their parent. -/
theorem S05_Def5_11_row_le_parent_of_verticalTwoBoxRemoval
    {n k : Nat} {lam : YoungDiagram n} {mu : YoungDiagram k}
    (h : S05_IsVerticalTwoBoxRemoval lam mu) (i : Nat) :
    youngRow mu i <= youngRow lam i :=
  row_le_parent_of_verticalTwoStripChild h i

/-- Definition 5.9 horizontal two-strip row condition. -/
theorem S05_Def5_11_next_parent_row_le_child_row_of_horizontalTwoBoxRemoval
    {n k : Nat} {lam : YoungDiagram n} {mu : YoungDiagram k}
    (h : S05_IsHorizontalTwoBoxRemoval lam mu) (i : Nat) :
    youngRow lam (i + 1) <= youngRow mu i :=
  next_parent_row_le_child_row_of_horizontalTwoStripChild h i

/-- Definition 5.9 vertical two-strip row condition. -/
theorem S05_Def5_11_parent_row_le_child_row_add_one_of_verticalTwoBoxRemoval
    {n k : Nat} {lam : YoungDiagram n} {mu : YoungDiagram k}
    (h : S05_IsVerticalTwoBoxRemoval lam mu) (i : Nat) :
    youngRow lam i <= youngRow mu i + 1 :=
  parent_row_le_child_row_add_one_of_verticalTwoStripChild h i

/-- Definition 5.9 row fact: in a vertical two-box removal, every row loses at
most one box. -/
theorem S05_Def5_11_row_diff_le_one_of_verticalTwoBoxRemoval
    {n k : Nat} {lam : YoungDiagram n} {mu : YoungDiagram k}
    (h : S05_IsVerticalTwoBoxRemoval lam mu) (i : Nat) :
    youngRow lam i - youngRow mu i <= 1 :=
