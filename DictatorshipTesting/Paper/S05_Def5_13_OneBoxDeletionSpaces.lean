...
abbrev S05_Def5_13_OneBoxDeletionTableaux {n : Nat}
...
abbrev S05_Def5_13_TableauMaxAt {n : Nat}
...
theorem S05_Def5_13_mem_iff {n : Nat}
    {lam : YoungDiagram (n + 1)} {u : YoungCell lam}
    {T : StandardYoungTableau lam} :
    T ∈ S05_Def5_13_OneBoxDeletionTableaux lam u <->
      S05_Def5_13_TableauMaxAt T u := by
  rfl

/-- Definition 5.13 set-level component: every standard tableau has a unique
maximum-entry cell. -/
theorem S05_Def5_13_existsUnique_tableauMaxAt {n : Nat}
    {lam : YoungDiagram (n + 1)} (T : StandardYoungTableau lam) :
    ∃! u : YoungCell lam, S05_Def5_13_TableauMaxAt T u := by
