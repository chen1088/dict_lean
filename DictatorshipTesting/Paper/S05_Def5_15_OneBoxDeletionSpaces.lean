...
/-!
Paper statement: Definition 5.15 (`def:one-box-deletion-spaces`)
...
/-- Definition 5.13, set-level tableau deletion space. -/
abbrev S05_Def5_15_OneBoxDeletionTableaux {n : Nat}
    (lam : YoungDiagram (n + 1)) (u : YoungCell lam) :
    Set (StandardYoungTableau lam) :=
  OneBoxDeletionTableaux lam u

/-- Definition 5.13, maximum-entry condition. -/
abbrev S05_Def5_15_TableauMaxAt {n : Nat}
...
theorem S05_Def5_15_mem_iff {n : Nat}
    {lam : YoungDiagram (n + 1)} {u : YoungCell lam}
    {T : StandardYoungTableau lam} :
    T ∈ S05_Def5_15_OneBoxDeletionTableaux lam u <->
      S05_Def5_15_TableauMaxAt T u := by
  rfl

/-- Definition 5.13 set-level component: every standard tableau has a unique
maximum-entry cell. -/
theorem S05_Def5_15_existsUnique_tableauMaxAt {n : Nat}
    {lam : YoungDiagram (n + 1)} (T : StandardYoungTableau lam) :
    ∃! u : YoungCell lam, S05_Def5_15_TableauMaxAt T u := by
