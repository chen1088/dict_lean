...
/-- Scalar lower bounds on all non-`U_1` blocks imply the matching
spectral gap. -/
theorem S05_blockLowerBound_block_lower_bound_implies_gap {n : Nat} (c : ℝ)
    (F : Perm (Fin n) -> ℝ)
...
/-- Internal alias for the direct weighted-sum comparison. -/
theorem S05_blockLowerBound_spectralGapFromBlockScalars {n : Nat} (c : ℝ)
    (F : Perm (Fin n) -> ℝ)
    (blockEnergy theta : YoungDiagram n -> ℝ)
    (hdecomp : YoungBlockDecompositionInput blockEnergy)
    (hu1 : U1YoungBlockIdentificationInput F blockEnergy)
...
/-- Internal alias: the trace/scalar formula plus the finite
certificate gives scalar lower bounds on non-`U_1` blocks. -/
theorem S05_blockLowerBound_blockScalar_lower_bound_of_traceScalarFormula {n : Nat}
...
/-- Internal alias: dimension positivity and block trace
identity imply the scalar value formula. -/
theorem S05_blockLowerBound_traceScalarValue_of_blockTraceIdentity {n : Nat}
...
/-- Internal alias: spectral gap from scalarity, block trace
identity, dimension positivity, and a finite certificate. -/
theorem S05_blockLowerBound_spectralGapFromBlockTraceModel {n : Nat} (c : ℝ)
    (F : Perm (Fin n) -> ℝ)
...
/-- Internal dimension-parameterized alias: trace/scalar formula plus a
finite certificate gives scalar lower bounds on non-`U_1` blocks. -/
theorem S05_blockLowerBound_blockScalar_lower_bound_withDim {n : Nat}
...
/-- Internal dimension-parameterized alias: positive dimension plus the block
trace identity imply the scalar value formula. -/
theorem S05_blockLowerBound_traceScalarValue_of_blockTraceIdentity_withDim {n : Nat}
...
/-- Internal dimension-parameterized alias: spectral gap from scalarity,
trace/scalar formula, and a certificate with respect to an arbitrary dimension
function. -/
theorem S05_blockLowerBound_spectralGapFromBlockScalarsWithDim {n : Nat} (c : ℝ)
    (F : Perm (Fin n) -> ℝ)
...
theorem S05_blockLowerBound_spectralGapFromBlockTraceModelWithDim {n : Nat} (c : ℝ)
    (F : Perm (Fin n) -> ℝ)
...
/-- Internal dimension-parameterized alias: a parameterized spectral-block
model plus a finite certificate imply the spectral gap. -/
theorem S05_blockLowerBound_spectralGapFromBlockModelWithDim {n : Nat}
