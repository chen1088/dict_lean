...
theorem S05_oneBoxDimension_tableauDim_fixed_oneBoxChild
...
theorem S05_oneBoxDimension_tableauDim_oneBox_branching
    {n : Nat} (lam : YoungDiagram (n + 1)) :
    tableauDim lam =
      (oneBoxChildrenSized lam).sum (fun mu => tableauDim mu) := by
  exact tableauDim_oneBox_branching_sized lam

theorem S05_oneBoxDimension_tableauDim_oneBoxChildrenOdd_branching
