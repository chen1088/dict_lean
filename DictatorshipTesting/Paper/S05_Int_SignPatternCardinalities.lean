...
theorem S05_signPatternCardinality_evenSignPatternMultiset_card :
...
tableau, with multiplicity. -/
theorem S05_signPatternCardinality_oddSignPatternMultiset_card
  ...
  calc
    ...
    _ = (oneBoxChildrenOdd m lam).sum (fun mu => tableauDim mu) := by
      exact Finset.sum_congr rfl
        (fun mu _ => S05_signPatternCardinality_evenSignPatternMultiset_card m mu)
    _ = tableauDim lam := by
      symm
      exact tableauDim_oneBoxChildrenOdd_branching m lam

@[simp] theorem S05_signPatternCardinality_liftEvenSignPattern_card
    {m : Nat} (R : Finset (Fin m)) :
    (S05_liftEvenSignPattern R).card = R.card := by
  simp [S05_liftEvenSignPattern]

@[simp] theorem S05_signPatternCardinality_liftEvenSignPatternWithLast_card
    {m : Nat} (R : Finset (Fin m)) :
...
private theorem highMultiplicity_map_liftWithLast
  ...
  intro R _hR
  rw [S05_signPatternCardinality_liftEvenSignPatternWithLast_card]
  omega
...
`zEven`. -/
theorem S05_signPatternCardinality_evenSignPatternMultiset_zeroMultiplicity :
    forall (m : Nat) (lam : YoungDiagram (2 * m)),
      ((S05_zeroSignPatternMultiplicity
...
theorem S05_signPatternCardinality_evenSignPatternMultiset_highMultiplicity :
    forall (m : Nat) (lam : YoungDiagram (2 * m)),
      ((S05_highSignPatternMultiplicity
        ...
        _ =
          ...
          · apply Finset.sum_congr rfl
            ...
            rw [highMultiplicity_map_liftWithLast_cast,
              ...
              S05_signPatternCardinality_evenSignPatternMultiset_zeroMultiplicity]
            rfl
...
theorem S05_signPatternCardinality_oddSignPatternMultiset_highMultiplicity
    (m : Nat) (lam : YoungDiagram (2 * m + 1)) :
    ((S05_highSignPatternMultiplicity
  ...
  calc
    ...
    _ = (oneBoxChildrenOdd m lam).sum (fun mu => hEvenTableau m mu) := by
      exact Finset.sum_congr rfl
        (fun mu _ => S05_signPatternCardinality_evenSignPatternMultiset_highMultiplicity m mu)
    _ = hOddTableau m lam := by
      rw [hOddTableau]

/-- A finite label enumeration counts high labels by the corresponding
multiset multiplicity. -/
theorem S05_signPatternCardinality_highLabelSum_eq_multiplicity
...
theorem S05_signPatternCardinality_highLabelCount_of_evenSignPatternMultiset
  ...
  exact S05_signPatternCardinality_evenSignPatternMultiset_highMultiplicity m lam

/-- An enumeration whose label multiset is Definition 5.10 has exactly
`hOddTableau` high labels. -/
theorem S05_signPatternCardinality_highLabelCount_of_oddSignPatternMultiset
    {m : Nat} {lam : YoungDiagram (2 * m + 1)}
    {ι : Type*} [Fintype ι]
    (label : ι -> Finset (Fin m))
...
theorem S05_signPatternCardinality_tableauDim_oneBoxChildrenOdd_size
  ...
  exact S05_oneBoxDimension_tableauDim_oneBoxChildrenOdd_branching m lam
