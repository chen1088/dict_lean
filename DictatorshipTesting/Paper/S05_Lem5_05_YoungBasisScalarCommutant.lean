...
-/
/-!
Paper statement: Lemma 5.5 (`lem:young-basis-scalar-commutant`)
...
The path is supplied internally by Lemma 5.4. -/
theorem YoungModelOperatorCommutationData.basisScalar_constant {n : Nat}
    {lam : YoungDiagram (n + 1)}
    (data : YoungModelOperatorCommutationData lam)
    (T U : StandardYoungTableau lam) :
    data.basisScalar T = data.basisScalar U := by
  have hpath := S05_Lem5_04_standardTableauxSwapConnectedness lam T U
...
the internally proved Lemma 5.4 makes it scalar on every tableau basis vector.
This does not instantiate the averaged matching rejection operator. -/
theorem S05_Lem5_05_youngModelOperator_scalar_on_basis {n : Nat}
    {lam : YoungDiagram (n + 1)}
    (data : YoungModelOperatorCommutationData lam)
...
/-- Lemma 5.5 interface projection: if the scalar commutant input holds, then
the averaged rejection is the weighted sum of block energies. -/
theorem S05_Lem5_05_matchingAverageScalarity_eq_sum {n : Nat}
    {F : Perm (Fin n) → ℝ}
