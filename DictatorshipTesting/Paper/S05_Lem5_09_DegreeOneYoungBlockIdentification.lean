...
-/
/-!
Paper statement: Lemma 5.9 (`lem:degree-one-young-blocks`)
...
orthogonal decomposition of Lemma 5.8.
-/
...
/-- Lemma 5.9: degree-one Young-block identification, proved through the
explicit permutation-coordinate decomposition. -/
theorem S05_Lem5_09_degreeOneYoungBlockIdentification
    {n : Nat}
...
/-- The faithful Lemma 5.9 subspace equality and the concrete orthogonal Young
...
/-- Lemma Lemma 5.9 numerical consequence for the concrete Young-block energies. -/
theorem S05_Lem5_09_l2DistSqToU1_eq_nonU1_sum
    {n : Nat}
    (action : ∀ lam : YoungDiagram (n + 1), YoungOrthogonalActionData lam)
    (content : ∀ lam : YoungDiagram (n + 1),
      JucysMurphyContentActionData (action lam))
  ...
  exact l2DistSqToU1_eq_sum_concreteYoungBlockEnergy action content
    (S05_Lem5_09_degreeOneYoungBlockIdentification action) F
