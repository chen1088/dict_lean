...
-/
/-!
Paper statement: Lemma 5.11 (`lem:one-box-corner-decomposition`)
...
/-- Lemma 5.11 row-form component: a one-box child differs from its parent in
exactly one row. -/
...
/-- Lemma 5.11 unique-row component. -/
...
/-- Lemma 5.11 removable-row component. -/
...
/-- Lemma 5.11 removable-corner component. -/
...
/-- Lemma 5.11 tableau component: the maximum entry lies in a removable corner. -/
...
/-- Lemma 5.11 tableau component: the maximum entry lies in a unique removable
corner. -/
...
/-- Lemma 5.11 row-form component, paper-numbered name. -/
theorem S05_Lem5_11_row_form
    {n k : Nat} {lam : YoungDiagram n} {mu : YoungDiagram k}
    (h : IsOneBoxChild lam mu) :
    exists r : Nat,
      youngRow lam r = youngRow mu r + 1 ∧
...
/-- Lemma 5.11 unique-row component, paper-numbered name. -/
theorem S05_Lem5_11_row_form_unique
    {n k : Nat} {lam : YoungDiagram n} {mu : YoungDiagram k}
    (h : IsOneBoxChild lam mu) :
    ∃! r : Nat,
      youngRow lam r = youngRow mu r + 1 ∧
...
/-- Lemma 5.11 removable-row component, paper-numbered name. -/
theorem S05_Lem5_11_removable_row
    {n k : Nat} {lam : YoungDiagram n} {mu : YoungDiagram k}
    (h : IsOneBoxChild lam mu) :
    exists r : Nat,
      IsRemovableRow lam r ∧
...
/-- Lemma 5.11 removable-corner component, paper-numbered name. -/
theorem S05_Lem5_11_removable_corner
    {n k : Nat} {lam : YoungDiagram n} {mu : YoungDiagram k}
    (h : IsOneBoxChild lam mu) :
    exists u : Nat × Nat,
      IsRemovableCornerBox lam u ∧
...
/-- Lemma 5.11 cell component, paper-numbered name: parent cells are child cells
or the deleted cell. -/
theorem S05_Lem5_11_parent_cell_iff_child_cell_or_deleted
    {n k : Nat} {lam : YoungDiagram n} {mu : YoungDiagram k}
...
/-- Lemma 5.11 cell component, paper-numbered name: child cells are parent cells
except the deleted cell. -/
theorem S05_Lem5_11_child_cell_iff_parent_cell_not_deleted
    {n k : Nat} {lam : YoungDiagram n} {mu : YoungDiagram k}
    (h : IsOneBoxChild lam mu) {r s c : Nat}
    (hr :
      youngRow lam r = youngRow mu r + 1 ∧
...
/-- Lemma 5.11 tableau component, paper-numbered name. -/
theorem S05_Lem5_11_tableau_max_removable_corner {n : Nat}
    {lam : YoungDiagram (n + 1)} (T : StandardYoungTableau lam)
    {u : YoungCell lam} (hu : TableauMaxAt T u) :
    IsRemovableCornerBox lam (YoungCell.toNatPair u) := by
  exact S05_oneBox_tableau_max_removable_corner T hu

/-- Lemma 5.11 tableau component, paper-numbered unique-corner name. -/
theorem S05_Lem5_11_tableau_unique_removable_corner {n : Nat}
    {lam : YoungDiagram (n + 1)} (T : StandardYoungTableau lam) :
    ∃! u : YoungCell lam,
      TableauMaxAt T u ∧ IsRemovableCornerBox lam (YoungCell.toNatPair u) := by
  exact S05_oneBox_tableau_unique_removable_corner T

/-- Lemma 5.11(a): deleting the maximum entry is a bijection on the remaining
tableau cells. -/
...
/-- Lemma 5.11(b): deletion preserves the coordinate inner product. -/
theorem S05_Lem5_11_deletionCoordinateMap_inner
...
/-- Lemma 5.11(c): deletion intertwines every earlier adjacent operator. -/
