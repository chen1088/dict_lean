...
/-- Lemma 5.16 scalar-size base case: the empty even diagram has one
zero-weight entry. -/
...
/-- Lemma 5.16 scalar-size component: the even zero-weight count has the
...
/-- Lemma 5.16 scalar-size base case: the empty even diagram has no high-weight
entries. -/
...
/-- Lemma 5.16 scalar-size component: the even high-weight count has the
...
/-- Lemma 5.16 scalar-size component: the odd high-weight count is the sum of
...
/-- Lemma 5.16 dimension-size component: the even two-strip dimension size
...
/-- Lemma 5.16 dimension-size component: the odd one-box dimension size
...
/-- Lemma 5.16 tableau-count component: the assumption-free two-strip size
recursion for standard-tableau counts. -/
theorem S05_Lem5_16_tableauDim_twoStrip_size
  ...
  exact S05_Lem5_14_tableauDim_twoStrip_branching_sized lam

/-- Lemma 5.16 tableau-count component: the assumption-free even two-strip size
recursion in the indexed Section 5 notation. -/
theorem S05_Lem5_16_tableauDim_twoStripChildrenEven_size_succ
    (m : Nat) (lam : YoungDiagram (2 * (m + 1))) :
    tableauDim lam =
      (horizontalTwoStripChildrenEven (m + 1) lam).sum
  ...
  exact S05_Lem5_14_tableauDim_twoStripChildrenEven_branching_succ m lam

/-- Lemma 5.16 tableau-count component: the assumption-free one-box size
recursion for standard-tableau counts. -/
theorem S05_Lem5_16_tableauDim_oneBox_size
    {n : Nat} (lam : YoungDiagram (n + 1)) :
    tableauDim lam =
      (oneBoxChildrenSized lam).sum (fun mu => tableauDim mu) := by
  exact S05_Lem5_15_tableauDim_oneBox_branching lam

/-- Lemma 5.16 tableau-count component: the assumption-free odd one-box size
recursion in the indexed Section 5 notation. -/
theorem S05_Lem5_16_tableauDim_oneBoxChildrenOdd_size
    (m : Nat) (lam : YoungDiagram (2 * m + 1)) :
    tableauDim lam =
      (oneBoxChildrenOdd m lam).sum (fun mu => tableauDim mu) := by
  exact S05_Lem5_15_tableauDim_oneBoxChildrenOdd_branching m lam
