...
/-!
Paper statement: Lemma 5.17 (`lem:PM-character-projection`)
...
/-- Lemma 5.17, matching-character vocabulary: local truncation preserves low
matching characters. -/
theorem S05_Lem5_17_cubeLowDegreeOnePart_matchingCharacter_of_low {m : ℕ}
...
/-- Lemma 5.17, matching-character vocabulary: local truncation kills high
matching characters. -/
theorem S05_Lem5_17_cubeLowDegreeOnePart_matchingCharacter_of_high {m : ℕ}
    {S : Finset (Fin m)} (hS : S05_matchingCharacterHigh S) :
    cubeLowDegreeOnePart (fun x : Cube m => S05_matchingCharacter S x) =
      fun _ : Cube m => 0 := by
  exact cubeLowDegreeOnePart_cubeChar_of_two_le_card hS

/-- Lemma 5.17, matching-character vocabulary: every matching character is
either preserved by the local truncation or killed by it. -/
theorem S05_Lem5_17_cubeLowDegreeOnePart_matchingCharacter_preserved_or_killed
    {m : ℕ} (S : Finset (Fin m)) :
    (cubeLowDegreeOnePart (fun x : Cube m => S05_matchingCharacter S x) =
  ...
  · exact Or.inl (S05_Lem5_17_cubeLowDegreeOnePart_matchingCharacter_of_low hlow)
  · exact Or.inr (S05_Lem5_17_cubeLowDegreeOnePart_matchingCharacter_of_high hhigh)

/-- Lemma 5.17, matching-character vocabulary: `P_M` preserves low local
matching characters. -/
theorem S05_Lem5_17_matchingLocalProjection_preserves_low_matchingCharacter
...
/-- Lemma 5.17, matching-character vocabulary: `P_M` kills high local matching
characters. -/
theorem S05_Lem5_17_matchingLocalProjection_kills_high_matchingCharacter
    {α : Type*} [Fintype α] [DecidableEq α] (M : OrderedMatching α)
    (F : Perm α → ℝ) (π : Perm α) {S : Finset (Fin M.edgeCount)}
