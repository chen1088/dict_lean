...
/-!
Paper statement: Lemma 5.18 (`lem:PM-trace-young-block`)
...
unconditionally from Lemma 5.16's arbitrary-matching bases.
-/
...
/-- Concrete operator reduction for Lemma 5.18: the fixed high-matching trace
is the sum of the traces of its high-character Fourier idempotents. -/
theorem S05_Lem5_18_fixedMatching_trace_eq_sum_characterTraces
...
/-- Concrete multiplicity reduction for Lemma 5.18: right action on the full
matrix-coordinate Young block contributes one identical trace for each left
tableau index. -/
theorem S05_Lem5_18_youngBlockTrace_eq_tableauDim_mul_repTrace
    {n : Nat} {lam : YoungDiagram (n + 1)}
    (action : YoungOrthogonalActionData lam)
    (M : NearPerfectMatching (n + 1)) :
...
5.10. -/
theorem S05_Lem5_18_fixedMatching_tableauTrace_even_of_eigenbasis
...
from exactly the matching eigenbasis and high-label count asserted in Lemma
5.10. -/
theorem S05_Lem5_18_fixedMatching_tableauTrace_odd_of_eigenbasis
...
theorem S05_Lem5_18_fixedMatching_youngBlockTrace_even_of_eigenbasis
    {n m : Nat} (hsize : n + 1 = 2 * m)
    {lam : YoungDiagram (n + 1)}
    (action : YoungOrthogonalActionData lam)
    (M : NearPerfectMatching (n + 1))
  ...
  rw [S05_Lem5_18_youngBlockTrace_eq_tableauDim_mul_repTrace action M]
  rw [S05_Lem5_18_fixedMatching_tableauTrace_even_of_eigenbasis
    hsize action M b label heigen hcount]

/-- Odd full Young-block trace formula.  The free left tableau index supplies
the factor `tableauDim`. -/
theorem S05_Lem5_18_fixedMatching_youngBlockTrace_odd_of_eigenbasis
    {n m : Nat} (hsize : n + 1 = 2 * m + 1)
    {lam : YoungDiagram (n + 1)}
    (action : YoungOrthogonalActionData lam)
    (M : NearPerfectMatching (n + 1))
  ...
  rw [S05_Lem5_18_youngBlockTrace_eq_tableauDim_mul_repTrace action M]
  rw [S05_Lem5_18_fixedMatching_tableauTrace_odd_of_eigenbasis
    hsize action M b label heigen hcount]

/-- Unconditional positive-size even tableau trace, using the transported
arbitrary-perfect-matching basis from Lemma 5.16. -/
theorem S05_Lem5_18_fixedMatching_tableauTrace_even
  ...
  exact S05_Lem5_18_fixedMatching_tableauTrace_even_of_eigenbasis
...
theorem S05_Lem5_18_fixedMatching_youngBlockTrace_even
    (m : Nat) (lam : YoungDiagram (2 * (m + 1)))
    (action : YoungOrthogonalActionData lam)
    (M : NearPerfectMatching (2 * (m + 1))) :
    youngBlockRightCoordinateTrace
  ...
  rw [S05_Lem5_18_youngBlockTrace_eq_tableauDim_mul_repTrace action M]
  rw [S05_Lem5_18_fixedMatching_tableauTrace_even m lam action M]

/-- Unconditional odd tableau trace, using the arbitrary near-perfect-matching
basis from Lemma 5.16. -/
theorem S05_Lem5_18_fixedMatching_tableauTrace_odd
    (m : Nat) (lam : YoungDiagram (2 * m + 1))
    (action : YoungOrthogonalActionData lam)
    (M : NearPerfectMatching (2 * m + 1)) :
    tableauOperatorTrace
  ...
  exact S05_Lem5_18_fixedMatching_tableauTrace_odd_of_eigenbasis
...
theorem S05_Lem5_18_fixedMatching_youngBlockTrace_odd
    (m : Nat) (lam : YoungDiagram (2 * m + 1))
    (action : YoungOrthogonalActionData lam)
    (M : NearPerfectMatching (2 * m + 1)) :
    youngBlockRightCoordinateTrace
  ...
  rw [S05_Lem5_18_youngBlockTrace_eq_tableauDim_mul_repTrace action M]
  rw [S05_Lem5_18_fixedMatching_tableauTrace_odd m lam action M]
