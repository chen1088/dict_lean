...
-/
/-!
Paper statement: Lemma 5.18 (`lem:z-bound-app`)
...
/-!
# Finite induction input for Lemma 5.18
...
the finite induction for Lemma 5.18. -/
...
/-- Finite Young-diagram induction behind Lemma 5.18. -/
...
/-- Lemma 5.18, `lem:z-bound-app`: weight-zero entries are never a majority.
...
/-- Lemma 5.18 paper-numbered alias: weight-zero entries are never a majority
outside the one-row block. -/
theorem S05_Lem5_18_weightZeroEntries_never_majority
    [TwoStripDimensionBranchingAssumption]
    (m : ℕ) (lam : YoungDiagram (2 * m))
    (hrow : ¬ IsOneRow lam) :
    zEven m lam ≤ (1 / 2 : ℝ) * youngDim lam := by
...
/-- Lemma 5.18 paper-numbered corollary: the zero-weight count is bounded by
the whole Young dimension. -/
theorem S05_Lem5_18_zEven_le_youngDim
    [TwoStripDimensionBranchingAssumption]
    (m : ℕ) (lam : YoungDiagram (2 * m)) :
    zEven m lam ≤ youngDim lam := by
...
They prove Lemma 5.18 for `tableauDim` using the tableau-count two-strip
recursion from Lemma 5.17 and explicit finite checks for the two exceptional
...
/-- Lemma 5.18, tableau-dimension version: weight-zero entries are never a
majority outside the one-row block. -/
theorem S05_Lem5_18_tableau_weightZeroEntries_never_majority
    (m : ℕ) (lam : YoungDiagram (2 * m))
    (hrow : ¬ IsOneRow lam) :
    zEven m lam ≤ (1 / 2 : ℝ) * tableauDim lam := by
