...
-/
/-!
Paper statement: Lemma 5.19 (`lem:h-even-app`)
...
/-!
# Finite induction proof for Lemma 5.19

The intended proof uses the weight-zero bound from Lemma 5.18, the `hEven` recurrence, the dimension
...
/-- Generic tableau-dimension induction step for Lemma 5.19, away from the
...
/-- Generic induction step for Lemma 5.19, away from the exceptional children.
...
/-- Finite Young-diagram induction behind Lemma 5.19, using actual tableau
counts. -/
...
/-- Lemma 5.19, tableau-count version of the even certificate. -/
theorem S05_Lem5_19_tableau_even_certificate
    (m : ℕ) (hm : 2 ≤ m)
    (lam : YoungDiagram (2 * m))
    (hrow : ¬ IsOneRow lam) (hstd : ¬ IsStandard lam) :
    (1 / 5 : ℝ) * tableauDim lam ≤ hEvenTableau m lam := by
...
/-- Finite Young-diagram induction behind Lemma 5.19. -/
...
/-- Lemma 5.19 paper-numbered alias: even certificate. -/
theorem S05_Lem5_19_even_certificate
    [TwoStripDimensionBranchingAssumption]
    (m : ℕ) (hm : 2 ≤ m)
    (lam : YoungDiagram (2 * m))
    (hrow : ¬ IsOneRow lam) (hstd : ¬ IsStandard lam) :
