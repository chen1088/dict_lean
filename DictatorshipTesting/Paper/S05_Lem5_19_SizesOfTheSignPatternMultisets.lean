...
import DictatorshipTesting.Paper.S05_Lem5_17_TwoBoxDimensionRecursion
import DictatorshipTesting.Paper.S05_Lem5_18_OneBoxDimensionRecursion
...
/-!
Paper statement: Lemma 5.19 (`lem:X-size`)
...
theorem S05_Lem5_19_evenSignPatternMultiset_card :
...
tableau, with multiplicity. -/
theorem S05_Lem5_19_oddSignPatternMultiset_card
  ...
  calc
    ...
    _ = (oneBoxChildrenOdd m lam).sum (fun mu => tableauDim mu) := by
      exact Finset.sum_congr rfl
        (fun mu _ => S05_Lem5_19_evenSignPatternMultiset_card m mu)
    _ = tableauDim lam := by
      symm
      exact tableauDim_oneBoxChildrenOdd_branching m lam

@[simp] theorem S05_Lem5_19_liftEvenSignPattern_card
    {m : Nat} (R : Finset (Fin m)) :
    (S05_liftEvenSignPattern R).card = R.card := by
  simp [S05_liftEvenSignPattern]

@[simp] theorem S05_Lem5_19_liftEvenSignPatternWithLast_card
    {m : Nat} (R : Finset (Fin m)) :
...
private theorem highMultiplicity_map_liftWithLast
  ...
  intro R _hR
  rw [S05_Lem5_19_liftEvenSignPatternWithLast_card]
  omega
...
`zEven`. -/
theorem S05_Lem5_19_evenSignPatternMultiset_zeroMultiplicity :
    forall (m : Nat) (lam : YoungDiagram (2 * m)),
      ((S05_zeroSignPatternMultiplicity
...
theorem S05_Lem5_19_evenSignPatternMultiset_highMultiplicity :
    forall (m : Nat) (lam : YoungDiagram (2 * m)),
      ((S05_highSignPatternMultiplicity
...
theorem S05_Lem5_19_oddSignPatternMultiset_highMultiplicity
    (m : Nat) (lam : YoungDiagram (2 * m + 1)) :
    ((S05_highSignPatternMultiplicity
  ...
  calc
    ...
    _ = (oneBoxChildrenOdd m lam).sum (fun mu => hEvenTableau m mu) := by
      exact Finset.sum_congr rfl
        (fun mu _ => S05_Lem5_19_evenSignPatternMultiset_highMultiplicity m mu)
    _ = hOddTableau m lam := by
      rw [hOddTableau]

/-- A finite label enumeration counts high labels by the corresponding
multiset multiplicity. -/
theorem S05_Lem5_19_highLabelSum_eq_multiplicity
...
/-- An enumeration whose label multiset is Definition 5.10 has exactly
`hEvenTableau` high labels. -/
theorem S05_Lem5_19_highLabelCount_of_evenSignPatternMultiset
    ...
    (∑ i : ι,
        if S05_matchingCharacterHigh (label i) then (1 : Real) else 0) =
      hEvenTableau m lam := by
  rw [S05_Lem5_19_highLabelSum_eq_multiplicity, hlabels]
  exact S05_Lem5_19_evenSignPatternMultiset_highMultiplicity m lam

/-- An enumeration whose label multiset is Definition 5.10 has exactly
`hOddTableau` high labels. -/
theorem S05_Lem5_19_highLabelCount_of_oddSignPatternMultiset
    {m : Nat} {lam : YoungDiagram (2 * m + 1)}
    {ι : Type*} [Fintype ι]
    (label : ι -> Finset (Fin m))
    ...
    (∑ i : ι,
      ...
      hOddTableau m lam := by
  rw [S05_Lem5_19_highLabelSum_eq_multiplicity, hlabels]
  exact S05_Lem5_19_oddSignPatternMultiset_highMultiplicity m lam

/-- Lemma 5.19 scalar-size base case: the empty even diagram has one
zero-weight entry. -/
theorem S05_Lem5_19_evenZeroSignPatternCount_zero
    (lam : YoungDiagram (2 * 0)) :
    S05_evenZeroSignPatternCount 0 lam = 1 := by
  exact S05_evenZeroSignPatternCount_zero lam

/-- Lemma 5.19 scalar-size component: the even zero-weight count has the
horizontal two-strip recursion. -/
theorem S05_Lem5_19_evenZeroSignPatternCount_succ
    (m : Nat) (lam : YoungDiagram (2 * (m + 1))) :
...
/-- Lemma 5.19 scalar-size base case: the empty even diagram has no high-weight
entries. -/
theorem S05_Lem5_19_evenHighSignPatternCount_zero
    (lam : YoungDiagram (2 * 0)) :
    S05_evenHighSignPatternCount 0 lam = 0 := by
  exact S05_evenHighSignPatternCount_zero lam

/-- Lemma 5.19 scalar-size component: the even high-weight count has the
horizontal-plus-vertical recursion. -/
theorem S05_Lem5_19_evenHighSignPatternCount_succ
    (m : Nat) (lam : YoungDiagram (2 * (m + 1))) :
...
/-- Lemma 5.19 scalar-size component: the odd high-weight count is the sum of
even high-weight counts over one-box removals. -/
theorem S05_Lem5_19_oddHighSignPatternCount_eq_evenHigh_sum
    (m : Nat) (lam : YoungDiagram (2 * m + 1)) :
...
/-- Lemma 5.19 dimension-size component: the even two-strip dimension size
recursion, supplied by the named branching input. -/
theorem S05_Lem5_19_youngDim_twoStrip_size
...
/-- Lemma 5.19 dimension-size component: the odd one-box dimension size
recursion, supplied by the named branching input except in the proved small
cases. -/
theorem S05_Lem5_19_youngDim_oneBox_size
...
/-- Lemma 5.19 tableau-count component: the assumption-free two-strip size
recursion for standard-tableau counts. -/
theorem S05_Lem5_19_tableauDim_twoStrip_size
  ...
  exact S05_Lem5_17_tableauDim_twoStrip_branching_sized lam

/-- Lemma 5.19 tableau-count component: the assumption-free even two-strip size
recursion in the indexed Section 5 notation. -/
theorem S05_Lem5_19_tableauDim_twoStripChildrenEven_size_succ
    (m : Nat) (lam : YoungDiagram (2 * (m + 1))) :
    tableauDim lam =
      (horizontalTwoStripChildrenEven (m + 1) lam).sum
          (fun mu => tableauDim mu) +
        (verticalTwoStripChildrenEven (m + 1) lam).sum
          (fun mu => tableauDim mu) := by
  exact S05_Lem5_17_tableauDim_twoStripChildrenEven_branching_succ m lam

/-- Lemma 5.19 tableau-count component: the assumption-free one-box size
recursion for standard-tableau counts. -/
theorem S05_Lem5_19_tableauDim_oneBox_size
    {n : Nat} (lam : YoungDiagram (n + 1)) :
    tableauDim lam =
      (oneBoxChildrenSized lam).sum (fun mu => tableauDim mu) := by
  exact S05_Lem5_18_tableauDim_oneBox_branching lam

/-- Lemma 5.19 tableau-count component: the assumption-free odd one-box size
recursion in the indexed Section 5 notation. -/
theorem S05_Lem5_19_tableauDim_oneBoxChildrenOdd_size
    (m : Nat) (lam : YoungDiagram (2 * m + 1)) :
    tableauDim lam =
      (oneBoxChildrenOdd m lam).sum (fun mu => tableauDim mu) := by
  exact S05_Lem5_18_tableauDim_oneBoxChildrenOdd_branching m lam
