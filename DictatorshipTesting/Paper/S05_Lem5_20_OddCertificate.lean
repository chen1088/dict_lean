...
-/
/-!
Paper statement: Lemma 5.20 (`lem:h-odd-app`)
...
/-!
# Finite induction proof for Lemma 5.20

The intended proof branches once to even diagrams, applies Lemma 5.19 to the
...
/-- Finite Young-diagram induction behind Lemma 5.20, using actual tableau
counts. -/
...
/-- Lemma 5.20, tableau-count version of the odd certificate. -/
theorem S05_Lem5_20_tableau_odd_certificate
    (m : ℕ) (hm : 2 ≤ m)
    (lam : YoungDiagram (2 * m + 1))
    (hrow : ¬ IsOneRow lam) (hstd : ¬ IsStandard lam) :
    (1 / 6 : ℝ) * tableauDim lam ≤ hOddTableau m lam := by
...
/-- Finite Young-diagram induction behind Lemma 5.20. -/
...
/-- Lemma 5.20 paper-numbered alias: odd certificate. -/
theorem S05_Lem5_20_odd_certificate
    [TwoStripDimensionBranchingAssumption]
    [OneBoxDimensionBranchingPositiveAssumption]
    (m : ℕ) (hm : 2 ≤ m)
    (lam : YoungDiagram (2 * m + 1))
