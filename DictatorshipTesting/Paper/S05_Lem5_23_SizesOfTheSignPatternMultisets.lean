import DictatorshipTesting.Paper.S05_Def5_20_OddSignPatternMultiset
import DictatorshipTesting.Paper.S05_Lem5_21_TwoBoxDimensionRecursion
...
/-!
Paper statement: Lemma 5.23 (`lem:X-size`)
...
zero-weight entry. -/
theorem S05_Lem5_23_evenZeroSignPatternCount_zero
...
theorem S05_Lem5_23_evenZeroSignPatternCount_succ
...
entries. -/
theorem S05_Lem5_23_evenHighSignPatternCount_zero
    (lam : YoungDiagram (2 * 0)) :
...
theorem S05_Lem5_23_evenHighSignPatternCount_succ
    (m : Nat) (lam : YoungDiagram (2 * (m + 1))) :
...
theorem S05_Lem5_23_oddHighSignPatternCount_eq_evenHigh_sum
    (m : Nat) (lam : YoungDiagram (2 * m + 1)) :
...
theorem S05_Lem5_23_youngDim_twoStrip_size
...
cases. -/
theorem S05_Lem5_23_youngDim_oneBox_size
...
theorem S05_Lem5_23_tableauDim_twoStrip_size
  ...
  exact S05_Lem5_21_tableauDim_twoStrip_branching_sized lam

/-- Lemma 5.21 tableau-count component: the assumption-free even two-strip size
recursion in the indexed Section 5 notation. -/
theorem S05_Lem5_23_tableauDim_twoStripChildrenEven_size_succ
    (m : Nat) (lam : YoungDiagram (2 * (m + 1))) :
    tableauDim lam =
      (horizontalTwoStripChildrenEven (m + 1) lam).sum
  ...
  exact S05_Lem5_21_tableauDim_twoStripChildrenEven_branching_succ m lam

/-- Lemma 5.21 tableau-count component: the assumption-free one-box size
recursion for standard-tableau counts. -/
theorem S05_Lem5_23_tableauDim_oneBox_size
    {n : Nat} (lam : YoungDiagram (n + 1)) :
    tableauDim lam =
      (oneBoxChildrenSized lam).sum (fun mu => tableauDim mu) := by
  exact S05_Lem5_22_tableauDim_oneBox_branching lam

/-- Lemma 5.21 tableau-count component: the assumption-free odd one-box size
recursion in the indexed Section 5 notation. -/
theorem S05_Lem5_23_tableauDim_oneBoxChildrenOdd_size
    (m : Nat) (lam : YoungDiagram (2 * m + 1)) :
    tableauDim lam =
      (oneBoxChildrenOdd m lam).sum (fun mu => tableauDim mu) := by
  exact S05_Lem5_22_tableauDim_oneBoxChildrenOdd_branching m lam
