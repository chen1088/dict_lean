...
theorem S05_Lem5_24_matchingLocalProjectionError_eq_high_idempotent_l2DistSq
    {α : Type*} [Fintype α] [DecidableEq α]
    (F : Perm α → ℝ) (M : OrderedMatching α) :
    matchingLocalProjectionError F M =
  ...
  intro π _hπ
  have hres := congrFun (S05_Lem5_23_residual_eq_high_idempotent M F) π
  rw [← hres]
  ring
...
theorem S05_Lem5_24_matchingMeanProjectionError_eq_high_idempotent_average
    {n : Nat} (F : Perm (Fin n) → ℝ) :
    matchingMeanProjectionError F =
      (∑ M : NearPerfectMatching n,
  ...
  apply Finset.sum_congr rfl
  intro M _hM
  exact S05_Lem5_24_matchingLocalProjectionError_eq_high_idempotent_l2DistSq
    F M.toOrdered
