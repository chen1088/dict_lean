...
/-- Lemma 5.28 finite-average component: the local rejection error is the
square distance from `F` to its matching-local projection. -/
theorem S05_Lem5_28_matchingLocalProjectionError_eq_l2DistSq
...
theorem S05_Lem5_28_matchingLocalProjectionError_eq_high_idempotent_l2DistSq
    {α : Type*} [Fintype α] [DecidableEq α]
    (F : Perm α → ℝ) (M : OrderedMatching α) :
    matchingLocalProjectionError F M =
  ...
  intro π _hπ
  have hres := congrFun (S05_Lem5_27_residual_eq_high_idempotent M F) π
  rw [← hres]
  ring

/-- Lemma 5.28 finite-average component: the averaged rejection is the uniform
...
theorem S05_Lem5_28_matchingMeanProjectionError_eq_average
...
theorem S05_Lem5_28_matchingMeanProjectionError_eq_high_idempotent_average
    {n : Nat} (F : Perm (Fin n) → ℝ) :
    matchingMeanProjectionError F =
      (∑ M : NearPerfectMatching n,
        l2DistSq (S05_matchingHighIdempotent M.toOrdered F) (fun _ => 0)) /
          (Fintype.card (NearPerfectMatching n) : ℝ) := by
  rw [S05_Lem5_28_matchingMeanProjectionError_eq_average]
  congr 1
  apply Finset.sum_congr rfl
  intro M _hM
  exact S05_Lem5_28_matchingLocalProjectionError_eq_high_idempotent_l2DistSq
    F M.toOrdered
