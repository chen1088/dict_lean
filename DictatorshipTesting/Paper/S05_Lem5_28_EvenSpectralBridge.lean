...
/-!
Paper statement: Lemma 5.28 (`lem:spectral-certificate-even`)
...
/-- Lemma 5.28: even finite Young-diagram inequalities imply the matching
spectral gap, assuming the stated spectral-block model. -/
theorem S05_Lem5_28_even_spectral_bridge
    (m : Nat) (hm : 2 ≤ m) (c : ℝ)
...
/-- Lemma 5.28 paper-numbered alias for the even spectral bridge before the
extra nonnegativity argument is threaded through. -/
theorem S05_Lem5_28_evenSpectralGapFromCertificates
    (m : Nat) (hm : 2 ≤ m) (c : ℝ)
    (hmodel : SpectralBlockModelInput
      (fun lam : YoungDiagram (2 * m) => hEven m lam))
    (hcert :
...
theorem S05_Lem5_28_tableauDim_evenSpectralGapFromCertificates
    (m : Nat) (hm : 2 <= m)
    (hmodel :
  ...
  exact
    SpectralGapFromBlockModelWithDim
      ...
      hmodel
      (fun lam hrow hstd =>
        S05_Lem5_33_tableau_even_certificate m hm lam hrow hstd)
