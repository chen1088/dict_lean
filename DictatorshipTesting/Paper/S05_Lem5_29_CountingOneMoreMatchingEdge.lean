...
/-!
Paper statement: Lemma 5.29 (`lem:counting-one-more-matching-edge`)
...
/-- Lemma 5.29: counting one more matching edge.  The theorem name is kept from
...
theorem S05_Lem5_29_counting_one_more_matching_edge (m : ℕ) (hm : 1 ≤ m)
    (lam : YoungDiagram (2 * m)) :
    zEven m lam =
        (horizontalTwoStripChildrenEven m lam).sum
          (fun mu => zEven (m - 1) mu) ∧
