...
import DictatorshipTesting.Paper.Defs.S05_Def5_13b_CertificateExceptionalPredicates
...
/-!
Paper statement: Lemma 5.31 (`lem:z-bound-app`)
Title in paper: Weight-zero entries are never a majority.

Status: proven. The tableau-count certificate is proved below. The `youngDim`
variant remains available as internal numerical machinery.
-/

/-!
# Finite induction input for Lemma 5.31
...
This file contains both the detailed finite induction and the paper-facing
tableau-dimension theorem.
-/
...
the finite induction for Lemma 5.31. -/
...
/-- Finite Young-diagram induction behind Lemma 5.31. -/
...
/-- Lemma 5.31, `lem:z-bound-app`: weight-zero entries are never a majority.
This internal variant uses the numerical `youngDim` model. -/
theorem S05_Int_ZBoundApp [TwoStripDimensionBranchingAssumption]
...
/-- Lemma 5.31 paper-numbered alias: weight-zero entries are never a majority
outside the one-row block. -/
theorem S05_Lem5_31_weightZeroEntries_never_majority
    [TwoStripDimensionBranchingAssumption]
    (m : ℕ) (lam : YoungDiagram (2 * m))
    (hrow : ¬ IsOneRow lam) :
    zEven m lam ≤ (1 / 2 : ℝ) * youngDim lam := by
  exact S05_Int_ZBoundApp m lam hrow

/-- Lemma 5.31 paper-numbered corollary: the zero-weight count is bounded by
the whole Young dimension. -/
theorem S05_Lem5_31_zEven_le_youngDim
    [TwoStripDimensionBranchingAssumption]
    (m : ℕ) (lam : YoungDiagram (2 * m)) :
    zEven m lam ≤ youngDim lam := by
...
They prove Lemma 5.31 for `tableauDim` using the tableau-count two-strip
recursion from Lemma 5.30 and explicit finite checks for the two exceptional
...
theorem tableauDim_standardDiagramEven_formula
    (m : ℕ) (hm : 1 ≤ m) :
    tableauDim (standardDiagramEven m hm) = 2 * (m : ℝ) - 1 := by
  induction m with
  | zero =>
      omega
  | succ m ih =>
      ...
      | succ k =>
          ...
          have ihstd := ih (by omega : 1 ≤ Nat.succ k)
          rw [ihstd]
          norm_num [Nat.succ_eq_add_one, Nat.cast_add]
          ring
...
theorem tableauDim_twoRowTwoDiagramEven_formula
    (m : ℕ) (hm : 2 ≤ m) :
    tableauDim (twoRowTwoDiagramEven m hm) =
      (m : ℝ) * (2 * (m : ℝ) - 3) := by
  induction m with
  | zero =>
      omega
...
/-- Lemma 5.31, tableau-dimension version: weight-zero entries are never a
majority outside the one-row block. -/
theorem S05_Lem5_31_tableau_weightZeroEntries_never_majority
    (m : ℕ) (lam : YoungDiagram (2 * m))
    (hrow : ¬ IsOneRow lam) :
    zEven m lam ≤ (1 / 2 : ℝ) * tableauDim lam := by
