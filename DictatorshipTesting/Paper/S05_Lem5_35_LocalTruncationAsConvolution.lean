...
import DictatorshipTesting.Paper.S05_Def5_34_MatchingIdempotents

/-
Direct reverse imports:
- `DictatorshipTesting`
- `DictatorshipTesting.Paper.S05_Lem5_36_CentralAveragedRejection`
-/


/-!
Paper statement: Lemma 5.35 (`lem:PM-convolution`)
...
theorem S05_Lem5_35_local_truncation_as_convolution
...
idempotent. -/
theorem S05_Lem5_35_local_truncation_eq_low_idempotent
    {α : Type*} [Fintype α] [DecidableEq α]
    (M : OrderedMatching α) (F : Perm α -> ℝ) :
    matchingLocalProjection M F = S05_matchingLowIdempotent M F := by
  exact (S05_Lem5_35_local_truncation_as_convolution M F).1

/-- Lemma 5.27 high-idempotent form: the residual `I - P_M` is the high
matching idempotent. -/
theorem S05_Lem5_35_residual_eq_high_idempotent
    {α : Type*} [Fintype α] [DecidableEq α]
    (M : OrderedMatching α) (F : Perm α -> ℝ) :
    (fun π => F π - matchingLocalProjection M F π) =
      S05_matchingHighIdempotent M F := by
  exact (S05_Lem5_35_local_truncation_as_convolution M F).2
