import DictatorshipTesting.Paper.S05_Lem5_33_EvenCertificate
...
- `DictatorshipTesting.Paper.S05_Lem5_34_OddExceptionalChildren`
-/


/-!
Paper statement: Lemma 5.35 (`lem:h-odd-app`)
...
/-!
# Finite induction proof for Lemma 5.35

The intended proof branches once to even diagrams, applies Lemma 5.33 to the
...
/-- Finite Young-diagram induction behind Lemma 5.35, using actual tableau
counts. -/
...
/-- Lemma 5.35, tableau-count version of the odd certificate. -/
theorem S05_Lem5_35_tableau_odd_certificate
    (m : ℕ) (hm : 2 ≤ m)
    (lam : YoungDiagram (2 * m + 1))
    (hrow : ¬ IsOneRow lam) (hstd : ¬ IsStandard lam) :
    (1 / 6 : ℝ) * tableauDim lam ≤ hOddTableau m lam := by
...
/-- Finite Young-diagram induction behind Lemma 5.35. -/
...
/-- Internal numerical `youngDim` variant of the odd certificate. -/
theorem S05_Int_HOddApp
    [TwoStripDimensionBranchingAssumption]
    [OneBoxDimensionBranchingPositiveAssumption]
    (m : ℕ) (hm : 2 ≤ m)
    (lam : YoungDiagram (2 * m + 1))
...
/-- Lemma 5.35 paper-numbered alias: odd certificate. -/
theorem S05_Lem5_35_odd_certificate
    [TwoStripDimensionBranchingAssumption]
    [OneBoxDimensionBranchingPositiveAssumption]
    (m : ℕ) (hm : 2 ≤ m)
    (lam : YoungDiagram (2 * m + 1))
    (hrow : ¬ IsOneRow lam) (hstd : ¬ IsStandard lam) :
    (1 / 6 : ℝ) * youngDim lam ≤ hOdd m lam := by
  exact S05_Int_HOddApp m hm lam hrow hstd
