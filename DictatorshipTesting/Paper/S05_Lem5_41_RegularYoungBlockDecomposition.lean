...
import DictatorshipTesting.Paper.S05_Def5_09_YoungBlockDecompositionInput
import DictatorshipTesting.Paper.S05_Def5_10_U1YoungBlockIdentificationInput
...
/-!
Paper statement: Lemma 5.41 (`lem:regular-young-block-decomposition`)
...
/-- Lemma 5.41 model projection: each Young-block energy is nonnegative. -/
theorem S05_Lem5_41_blockEnergy_nonnegative {n : Nat}
    {F : Perm (Fin n) → ℝ}
...
/-- Lemma 5.41 model projection: the distance to `U_1` is the sum of all
non-`U_1` Young-block energies. -/
theorem S05_Lem5_41_u1_identification {n : Nat}
    {F : Perm (Fin n) → ℝ}
    (energy : YoungBlockEnergyModel F) :
    l2DistSqToU1 F =
...
/-- Lemma 5.41 assembly theorem for the tableau-count even route, from the
...
/-- Lemma 5.41 assembly theorem for the tableau-count odd route, from the
explicit classical Appendix A ingredients.
