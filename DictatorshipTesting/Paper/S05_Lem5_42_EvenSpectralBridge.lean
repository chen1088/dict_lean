...
/-!
Paper statement: Lemma 5.42 (`lem:spectral-certificate-even`)
...
theorem S05_Lem5_42_even_spectral_bridge
    (m : Nat) (hm : 2 ≤ m) (c : ℝ)
...
theorem S05_Lem5_42_evenSpectralGapFromCertificates
    (m : Nat) (hm : 2 ≤ m) (c : ℝ)
    (hmodel : SpectralBlockModelInput
      (fun lam : YoungDiagram (2 * m) => hEven m lam))
    (hcert :
...
theorem S05_Lem5_42_tableauDim_evenSpectralGapFromCertificates
    (m : Nat) (hm : 2 <= m)
    (hmodel :
  ...
  exact
    SpectralGapFromBlockModelWithDim
      ...
      hmodel
      (fun lam hrow hstd =>
        S05_Lem5_47_tableau_even_certificate m hm lam hrow hstd)
