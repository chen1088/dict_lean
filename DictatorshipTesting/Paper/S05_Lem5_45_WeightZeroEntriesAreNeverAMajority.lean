...
/-!
Paper statement: Lemma 5.45 (`lem:z-bound-app`)
...
theorem S05_Lem5_45_weightZeroEntries_never_majority
    [TwoStripDimensionBranchingAssumption]
    (m : ℕ) (lam : YoungDiagram (2 * m))
    (hrow : ¬ IsOneRow lam) :
    zEven m lam ≤ (1 / 2 : ℝ) * youngDim lam := by
...
theorem S05_Lem5_45_zEven_le_youngDim
    [TwoStripDimensionBranchingAssumption]
    (m : ℕ) (lam : YoungDiagram (2 * m)) :
    zEven m lam ≤ youngDim lam := by
...
theorem S05_Lem5_45_tableau_weightZeroEntries_never_majority
    (m : ℕ) (lam : YoungDiagram (2 * m))
    (hrow : ¬ IsOneRow lam) :
    zEven m lam ≤ (1 / 2 : ℝ) * tableauDim lam := by
