...
- `DictatorshipTesting.Paper.S05_Lem5_48_OddExceptionalChildren`
-/


/-!
Paper statement: Lemma 5.49 (`lem:h-odd-app`)
...
theorem S05_Lem5_49_tableau_odd_certificate
    (m : ℕ) (hm : 2 ≤ m)
    (lam : YoungDiagram (2 * m + 1))
    (hrow : ¬ IsOneRow lam) (hstd : ¬ IsStandard lam) :
    (1 / 6 : ℝ) * tableauDim lam ≤ hOddTableau m lam := by
...
theorem S05_Lem5_49_odd_certificate
    [TwoStripDimensionBranchingAssumption]
    [OneBoxDimensionBranchingPositiveAssumption]
    (m : ℕ) (hm : 2 ≤ m)
    (lam : YoungDiagram (2 * m + 1))
