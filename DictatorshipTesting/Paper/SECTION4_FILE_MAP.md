...
| external: Appendix A spectral model input | 4.10 | theorem | Matching-cube spectral gap | `S04_Thm4_10_MatchingGap.lean` | `Thm4_10_MatchingGap` | The active Lean route uses the Appendix A even/odd spectral model inputs and carries explicit `youngDim` dimension-branching typeclass hypotheses; no registered instances are provided for those dimension hypotheses. |
