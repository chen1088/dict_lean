...
| external: Appendix A spectral model input | 4.10 | theorem | Matching-cube spectral gap | `S04_Thm4_10_MatchingGap.lean` | `Thm4_10_MatchingGap` | The active Lean route uses the tableauDim Section 5 bridge and the dimension-parameterized Appendix A spectral model inputs; it no longer carries the older `youngDim` dimension-branching typeclass hypotheses. |
