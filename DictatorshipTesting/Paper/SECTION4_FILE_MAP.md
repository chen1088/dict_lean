...
| proven | 4.10 | theorem | Matching-cube spectral gap | `S04_Thm4_10_MatchingGap.lean` | `Thm4_10_MatchingGap` | Proved through the tableauDim Section 5 bridge. The remaining representation-theoretic dependency is the dimension-parameterized Appendix A spectral model input; the active route no longer carries the older `youngDim` dimension-branching typeclass hypotheses. |
