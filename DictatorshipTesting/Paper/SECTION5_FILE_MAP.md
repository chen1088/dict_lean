...
| external: block trace model input | 5.25 | lemma | Trace of one local truncation on one Young block | `S05_Lem5_25_TraceOfOneLocalTruncationOnOneYoungBlock.lean` | `traceLocalTruncation_even_from_restriction`, `traceLocalTruncation_odd_from_restriction` | The matching-character/local-truncation calculation is proved; the Young-block trace-space model is part of the spectral-block model boundary. |
