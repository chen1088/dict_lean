...
- Superseded: the operator-level braid blocker is closed.  Lemma 5.1 now has a
  coordinate Coxeter package for the concrete Young adjacent operators.
  `Aux_YoungAdjacentAction.lean` defines `YoungAdjacentActionModel`,
  `AdjacentWord`, `youngAdjacentWordOperator`, elementary Coxeter moves,
  `AdjacentWord.CoxeterEquiv`, the adjacent-word permutation shadow
  `adjacentWordPerm`, and the quotient `AdjacentCoxeterClass`.  The operator
  and permutation shadow both descend to this quotient.
- The remaining Lemma 5.1 boundary is not a tableau-coefficient calculation:
  it is the classical identification of this coordinate Coxeter model with the
  Specht-module/Young orthogonal representation, or equivalently a type-A
  Coxeter presentation theorem identifying the adjacent-word quotient with
  `Equiv.Perm (Fin (n+1))`.  Mathlib has abstract Coxeter matrices/systems and
  proves adjacent swaps generate finite symmetric groups, but this checkout
  does not currently provide a ready `CoxeterSystem (CoxeterMatrix.A n)
  (Equiv.Perm (Fin (n+1)))` or Matsumoto-style theorem saying any two adjacent
  words for the same permutation are related by our `AdjacentWord.CoxeterEquiv`.

Next automatically selected task:
- If staying inside Lemma 5.1, formalize the type-A symmetric-group Coxeter
  presentation or connect to mathlib's abstract `CoxeterSystem` API.  Otherwise
  the next representation-layer target is Lemma 5.19, using Lemma 5.1 only
  through the proved coordinate Coxeter model.

### Lemma 5.1 Coxeter/action package

Pushed in this sprint:
- `8532edc` Package Young adjacent Coxeter action model
- `376db82` Descend Young adjacent action to Coxeter word classes

Main names added:
- `YoungAdjacentActionModel`
- `youngAdjacentActionModel`
- `AdjacentWord`
- `youngAdjacentWordOperator`
- `youngAdjacentWordOperator_append`
- `youngAdjacentWordOperator_cancel_pair`
- `youngAdjacentWordOperator_swap_disjoint`
- `youngAdjacentWordOperator_braid_move_of_succ`
- `AdjacentWord.CoxeterStep`
- `AdjacentWord.CoxeterEquiv`
- `youngAdjacentWordOperator_respects_coxeter_step`
- `youngAdjacentWordOperator_respects_coxeter_equiv`
- `adjacentSwapPerm`
- `adjacentWordPerm`
- `adjacentWordPerm_append`
- `adjacentWordPerm_respects_coxeter_step`
- `adjacentWordPerm_respects_coxeter_equiv`
- `AdjacentCoxeterClass`
- `youngAdjacentCoxeterClassOperator`
- `adjacentCoxeterClassPerm`
- `S05_Lem5_01_youngAdjacentActionModel`
- `S05_Lem5_01_youngAdjacentWordOperator_respects_coxeter_equiv`
- `S05_Lem5_01_adjacentWordPerm_respects_coxeter_equiv`
- `S05_Lem5_01_adjacentTranspositionsInYoungsBasis_coxeterModel`

Lemma 5.1 current status:
- Same-row, same-column, and swappable matrix formulas are exposed in
  `S05_Lem5_01_youngAdjacent_matrices`.
- The genuine two-tableau block has nonzero off-diagonal coefficient and
  nonzero `+1`/`-1` eigenvectors, exposed in
  `S05_Lem5_01_youngAdjacent_swappable_eigenlines`.
- The concrete adjacent operators are involutions, commute at disjoint indices,
  and satisfy the successor braid relation on the full tableau coordinate
  space.
- The word action respects the Coxeter equivalence generated by cancellation,
  distant commutation, and braid moves.
- The adjacent-word permutation shadow into `Equiv.Perm (Fin (n+1))` respects
  the same Coxeter equivalence and descends to `AdjacentCoxeterClass`.
- A full `Perm (Fin (n+1))` action is not claimed.  The precise missing
  theorem is the type-A symmetric-group Coxeter presentation/Matsumoto
  equivalence for adjacent words, or a usable mathlib `CoxeterSystem`
  identifying `CoxeterMatrix.A n` with `Equiv.Perm (Fin (n+1))`.
